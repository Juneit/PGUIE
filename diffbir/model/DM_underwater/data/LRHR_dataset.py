...
class LRHRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 获取输入图像的文件名（不包含路径和扩展名）
        hr_filename = os.path.splitext(os.path.basename(self.hr_path[index]))[0]
        
        if self.need_LR:
            [img_LR, img_SR, img_HR] = Util.transform_augment(
                [img_LR, img_SR, img_HR], split=self.split, min_max=(-1, 1))
            return {'LR': img_LR, 'HR': img_HR, 'SR': img_SR, 'Index': index, 'HR_filename': hr_filename}
        else:
            [img_SR, img_HR] = Util.transform_augment(
                [img_SR, img_HR], split=self.split, min_max=(-1, 1))
            return {'HR': img_HR, 'SR': img_SR, 'Index': index, 'HR_filename': hr_filename}
...
class LRHRDataset2(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_HR = Image.open(self.hr_path[index]).convert("RGB").resize((256, 256))
        img_SR = Image.open(self.sr_path[index]).convert("RGB").resize((256, 256))
        # img_style = Image.open(self.style_path[index]).convert("RGB")
        img_style = Image.open(self.sr_path[index]).convert("RGB").resize((256, 256))
        if self.need_LR:
            img_LR = Image.open(self.lr_path[index]).convert("RGB")
        ...
        if self.need_LR:
            ...
            return {'LR': img_LR, 'HR': img_HR, 'SR': img_SR, 'style': img_style, 'Index': index, 'HR_filename': hr_filename}
        else:
            [img_SR, img_HR, img_style] = Util.transform_augment(
                [img_SR, img_HR, img_style], split=self.split, min_max=(-1, 1))
            return {'HR': img_HR, 'SR': img_SR, 'style': img_style, 'Index': index, 'HR_filename': hr_filename}
