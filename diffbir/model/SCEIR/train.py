...
class SCEIR_Model(nn.Module):
    ...
    def forward(self, batch):
        ...
        ratio = raw.mean(dim=(1,2,3)).reshape(-1,1,1) / raw[:, 0].mean(dim=(1, 2), keepdim=True)
        pre_mean[:,0] *= ratio
        ain_out = pre_std * (raw - raw_mean) / (raw_std + 1e-6) + pre_mean

        # concat gf_v
        # data['raw_v'] = data['raw'].max(dim=1, keepdim=True)[0]
        raw_v = raw.max(dim=1, keepdim=True)[0]
        gf_v = raw_v * 2 - self.guide_filter(raw_v, raw_v)
